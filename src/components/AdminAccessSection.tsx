...
const AdminAccessSection: FC<AdminAccessSectionProps> = ({setActiveView, userData}) => {
    ...
    const modificationHistory = userData.modificationHistory || [];
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-200 theme-athlete:border-slate-700">
                        ...
                        <div className={`collapsible-content ${isHistoryVisible ? 'open' : ''}`}>
                            <ul className="mt-3 space-y-2 text-xs text-slate-500">
                                {[...modificationHistory].reverse().slice(0, 5).map((entry, index) => (
                            ...
                            {modificationHistory.length > 5 && <p className="text-center text-xs text-slate-400 mt-2">Exibindo as 5 alterações mais recentes.</p>}
                        </div>
                    </div>
                )}
            </div>
