...
const WeightChart: FC<{ data: { date: string; weight: number }[], goal: number }> = ({ data, goal }) => {
    ...
    // Path for the line
    const path = chartData.map((d, i) => `${i === 0 ? 'M' : 'L'} ${getX(dates[i])} ${getY(d.weight)}`).join(' ');
    
    // Path for the gradient area
    const areaPath = `${path} L ${getX(dates[chartData.length - 1])} ${height - padding.bottom} L ${getX(dates[0])} ${height - padding.bottom} Z`;
    ...
    return (
        <div className="relative">
             <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto" role="img" aria-label="Gráfico de evolução do peso">
                ...
                {/* Main Path */}
                {chartData.length > 1 && <path d={path} fill="none" className="stroke-brand-green theme-athlete:stroke-accent-red" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />}
                
                {/* X-Axis Labels */}
                <text x={padding.left} y={height - padding.bottom + 15} className="fill-slate-500 theme-athlete:fill-zinc-400" fontSize="10">{new Date(chartData[0].date).toLocaleDateString('pt-BR', {day: '2-digit', month: 'short'})}</text>
                {chartData.length > 1 && <text x={width - padding.right} y={height - padding.bottom + 15} className="fill-slate-500 theme-athlete:fill-zinc-400" fontSize="10" textAnchor="end">{new Date(chartData[chartData.length - 1].date).toLocaleDateString('pt-BR', {day: '2-digit', month: 'short'})}</text>}
...
const ProgressView: React.FC<{ userData: UserData; handlers: UserDataHandlers; }> = ({ userData, handlers }) => {
    ...
    const registrationDate = useMemo(() => ((weightHistory || []).length > 0) ? (weightHistory || [])[0].date : new Date().toISOString(), [weightHistory]);
